...
/**
 * main.php
 * Premium Sidebar Navigation Layout
 */
...
<div class="flex h-full no-print">
    ...
    <aside class="hidden lg:flex lg:flex-col w-72 sidebar-premium h-full fixed inset-y-0 z-50 shadow-xl">
        ...
        <div class="px-4 mb-6">
            <div class="group relative bg-white/5 rounded-2xl p-4 border border-white/10 flex items-center gap-3 transition-all duration-300 hover:bg-white/10">
                <div class="w-12 h-12 rounded-xl bg-slate-800 overflow-hidden ring-2 ring-brand-500/20 p-0.5 group-hover:ring-brand-500/50 transition-all">
                    <?php if (!empty($_SESSION['auth']['profile_image'])): ?>
                        <img src="<?= url($_SESSION['auth']['profile_image']) ?>" class="w-full h-full object-cover rounded-lg">
                    <?php else: ?>
                        <div class="w-full h-full flex items-center justify-center bg-slate-800 text-slate-500 rounded-lg text-lg font-black">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5"><?= htmlspecialchars($currentRole) ?></p>
                </div>
                
                <!-- Hover Dropdown (Sidebar) -->
                <div class="absolute left-full ml-2 top-0 w-48 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-x-2 group-hover:translate-x-0 z-[60]">
                    <div class="bg-[#0f172a] rounded-2xl shadow-2xl border border-white/10 overflow-hidden p-1.5 backdrop-blur-xl">
                        <a href="<?= url('/profile') ?>" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-xs font-bold text-slate-300 hover:bg-white/5 hover:text-white transition-all">
                        ...
                        <a href="<?= url('/settings') ?>" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-xs font-bold text-slate-300 hover:bg-white/5 hover:text-white transition-all">
                        ...
                        <a href="<?= url('/logout') ?>" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-xs font-bold text-red-400 hover:bg-red-500/10 transition-all">
        ...
        <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
            ...
            <!-- MAIN SECTION -->
            <a href="<?= url('/dashboard') ?>" class="nav-link <?= navActive('/dashboard', $currentPath) ?> flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 text-sm font-medium">
            ...
            <?php if (in_array($currentRole, ['superadmin', 'vc', 'dean', 'hod', 'lecturer'])): ?>
                <div class="pt-6 pb-1 px-3"><p class="text-slate-500 text-[10px] font-bold uppercase tracking-widest">Academic Staff</p></div>
                
                <?php if (in_array($currentRole, ['superadmin', 'vc', 'dean', 'hod'])): ?>
                <a href="<?= url('/faculties') ?>" class="nav-link <?= navActive('/faculties', $currentPath) ?> flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 text-sm font-medium">
                ...
                <a href="<?= url('/departments') ?>" class="nav-link <?= navActive('/departments', $currentPath) ?> flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 text-sm font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M4 6h16M4 10h16M4 14h16M4 18h16"/></svg>
                    <span>Departments</span>
                </a>
                <?php endif; ?>

                <a href="<?= url('/submissions') ?>" class="nav-link <?= navActive('/submissions', $currentPath) ?> flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 text-sm font-medium">
                ...
                <a href="<?= url('/courses') ?>" class="nav-link <?= navActive('/courses', $currentPath) ?> flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 text-sm font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
                    <span>Courses</span>
                </a>
                <a href="<?= url('/results') ?>" class="nav-link <?= navActive('/results', $currentPath) ?> flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 text-sm font-medium">
            ...
            <?php if ($currentRole === 'student'): ?>
                <div class="pt-6 pb-1 px-3"><p class="text-slate-500 text-[10px] font-bold uppercase tracking-widest">Student Portal</p></div>
                <a href="<?= url('/my-courses') ?>" class="nav-link <?= navActive('/my-courses', $currentPath) ?> flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 text-sm font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
                ...
                <a href="<?= url('/records/transcript') ?>" class="nav-link <?= navActive('/records/transcript', $currentPath) ?> flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 text-sm font-medium">
                ...
                <a href="<?= url('/records/certificate') ?>" class="nav-link <?= navActive('/records/certificate', $currentPath) ?> flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 text-sm font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14zm-4 6v-7.5l4-2.222"/></svg>
            ...
            <?php if (in_array($currentRole, ['superadmin', 'vc', 'dean', 'hod'])): ?>
                <div class="pt-6 pb-1 px-3"><p class="text-slate-500 text-[10px] font-bold uppercase tracking-widest">Administration</p></div>
                <a href="<?= url('/users') ?>" class="nav-link <?= navActive('/users', $currentPath) ?> flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 text-sm font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                    <span>Users</span>
                </a>
            <?php endif; ?>
            ...
            ?>
                <div class="pt-6 pb-1 px-3"><p class="text-slate-500 text-[10px] font-bold uppercase tracking-widest">Admin Units</p></div>
                <?php if ($canSeeRegistry): ?>
                <a href="<?= url('/registry') ?>" class="nav-link <?= navActive('/registry', $currentPath) ?> flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 text-sm font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2"/></svg>
                    <span>Registry</span>
                </a>
                <?php endif; ?>
                <?php if ($canSeeBursary): ?>
                <a href="<?= url('/bursary') ?>" class="nav-link <?= navActive('/bursary', $currentPath) ?> flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 text-sm font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.407 2.67 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.407-2.67-1M12 16v1m-7-4h14a2 2 0 002-2V9a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2z"/></svg>
                    <span>Bursary</span>
                </a>
                <?php endif; ?>
                <?php if ($canSeeLibrary): ?>
                <a href="<?= url('/library') ?>" class="nav-link <?= navActive('/library', $currentPath) ?> flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 text-sm font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
                    <span>Library</span>
                </a>
                <?php endif; ?>
    ...
    <main class="lg:pl-72 flex-1 min-h-screen">
        <!-- Top Header -->
        <header class="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-40 px-8 py-4 flex items-center justify-between no-print">
            ...
            <div class="flex items-center gap-8">
                ...
                <div class="group relative flex items-center gap-4 pl-8 border-l border-slate-200 cursor-pointer py-2">
                    <div class="text-right hidden sm:block">
                        <p class="text-sm font-black text-slate-900 leading-none group-hover:text-brand-600 transition-colors"><?= htmlspecialchars($_SESSION['auth']['name'] ?? 'User') ?></p>
                    ...
                    <div class="absolute top-full right-0 mt-2 w-56 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 z-50">
                        <div class="bg-white rounded-[2rem] shadow-2xl shadow-slate-200 border border-slate-100 overflow-hidden p-2 backdrop-blur-xl bg-white/95">
                            <div class="px-4 py-3 border-b border-slate-50 mb-1">
                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Account Options</p>
                            </div>
